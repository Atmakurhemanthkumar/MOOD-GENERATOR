...
function updateSection(sectionId, items, isColor = false) {
  ...
  if (sectionId === 'animation-section') {
    // Special handling for animations with emojis
    items.forEach(item => {
      const div = document.createElement('div');
      div.className = 'item';
      div.classList.add('animated-text');
      div.style.animationName = item.name;
      div.innerText = item.emoji; // Use emoji instead of text
      
      // Add tooltip
      const tooltip = document.createElement('span');
      tooltip.className = 'tooltip';
      tooltip.innerText = `${item.name} - Click to copy`;
